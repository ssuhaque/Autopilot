function varargout = heading(varargin)
% HEADING Application M-file for heading.fig
%    FIG = HEADING launch heading GUI.
%    HEADING('callback_name', ...) invoke the named callback.

% Last Modified by GUIDE v2.0 02-Jul-2002 02:50:27
...
if nargin == 0  % LAUNCH GUI
    ...
    global tfrs;
    tfrs.tfdata=emptytfdata;
    tfrs.tf=(1);
    
    global tfdg;
    tfdg.tfdata=emptytfdata;
    tfdg.tf=(1);
    ...
    global tfad;
    tfad=drtf('r','davar'); %tf(3,[1,12,25,50]);
...
% --------------------------------------------------------------------
function varargout = tfdg_Callback(h, eventdata, handles, varargin)
% Stub for Callback of the uicontrol handles.tfdg.
global tfdg;
global reqtf;
reqtf=tfdg;
tfunction;
...
% --------------------------------------------------------------------
function varargout = tfrs_Callback(h, eventdata, handles, varargin)
% Stub for Callback of the uicontrol handles.tfrs.
...
if sys==1 
    h=pid(handles)*tfrs.tf*tfad;
    %  *tf(1,[1,0]);
    g=tfdg.tf;
    tfsys=FEEDBACK(h,g);
    autoTF.heading=tfsys;
end
