function varargout = altitude(varargin)
% ALTITUDE Application M-file for altitude.fig
%    FIG = ALTITUDE launch altitude GUI.
%    ALTITUDE('callback_name', ...) invoke the named callback.

% Last Modified by GUIDE v2.0 02-Jul-2002 02:54:36
...
% --------------------------------------------------------------------
function varargout = tfes_Callback(h, eventdata, handles, varargin)
% Stub for Callback of the uicontrol handles.tfes.
global tfes;
global reqtf;
reqtf=tfes;
tfunction;
...
if get(handles.system,'Value')==2 
   [x,map] = imread('comp-altitude','bmp');
end
image(x,'Parent',handles.axes2)
...
if sys==1 
    h=pid(handles)*tfes.tf*tfad;
    g=tfas.tf;
    tfsys=FEEDBACK(h,g);
    autoTF.altitude=tfsys;
end

if sys==2 
    h=tfampc.tf*tfes.tf*tfad;
    g=tfas.tf;
    tfsys=FEEDBACK(h,g);
    autoTF.altitude=tfsys;
end
...
% --------------------------------------------------------------------
function tfad=cal_ad(handles)
global dataStruct;

tf1=sptf('q','davae')*tf(1,[1,0]);
tf2=sptf('alpha','davae');
tfad=tf(dataStruct.Uo,[1,0])*( tf1-tf2 );
%tf(3,[1,12,25,50]);
